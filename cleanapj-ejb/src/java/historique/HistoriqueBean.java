...
import affichage.CNAPSUser;
/**
 * Implémentation de Historique, permettant de manipuler les données
 *  d'historique {@link historique.MapHistorique} et des informations
 * d'utilisateur {@link historique.MapUtilisateur}
 * 
 * @author BICI
 */
@Stateless
public class HistoriqueBean implements HistoriqueRemote, HistoriqueLocal, SessionBean {
    ...
    /**
     * trouver des historiques avec filtres
     * @param refuser id de l'utilisateu concerné
     * @param refObjet id de l'objet à voir
     * @param objet nom de la table modifiée
     * @param action action réalisée sur l'objet
     * @param daty1 date min 
     * @param daty2 datex max pour la recherche
    ...
    /**
     * trouver des historiques avec filtres et pagination acceptée
     * @param refuser id de l'utilisateu concerné
     * @param refObjet id de l'objet à voir
     * @param objet nom de la table modifiée
     * @param action action réalisée sur l'objet
    ...
    /**
     * calculer le nombre total de resultats de la recherche
     * @param refuser id de l'utilisateu concerné
     * @param refObjet id de l'objet à voir
     * @param objet nom de la table modifiée
     * @param action action réalisée sur l'objet
     ...
     * @param rol id du role à rechercher
     */
    ...
    public String createUtilisateurs(String loginuser, String pwduser, String nomuser, String adruser, String teluser, String idrole, String refUser) throws Exception {
        ...
        try {
            ...
            CNAPSUser cu=new CNAPSUser("DIR00001", "1", "1", "1", nomuser, "2030", u.getTuppleID());
            cu.construirePK(c);
            cu.insertToTableWithHisto(u.getTuppleID(), c);
            c.commit();
...
//    MapUtilisateur u=new MapUtilisateur(utilitaire.Utilitaire.stringToInt(iduser),loginuser, pwduser, nomuser, adruser, teluser, idrole);
        try {
            ...
            MapUtilisateur[] utils = (MapUtilisateur[])CGenUtil.rechercher(new MapUtilisateur(), null, null, " AND REFUSER = "+refUser+"");
            if(utils.length>0 && utils[0].getLoginuser().compareToIgnoreCase(loginuser)!=0){
                MapUtilisateur[] util = (MapUtilisateur[])CGenUtil.rechercher(new MapUtilisateur(), null, null, " AND LOGINUSER = '"+loginuser+"'");
     ...
     * @param refUser id de l'utilisateur qui a fait la desactivation
     * @return 1
     */
     ...
     * @param ref id de l'utilisateur à activer
     * @param refUser id de l'utilisateur qui fait l'activation
     * @return 1
     */
    public int activeUtilisateur(String ref, String refUser) throws bean.ErreurDAO {
        try {
            ...
            Logger.getLogger(HistoriqueBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return 1;
    }
    /**
    ...
    public MapUtilisateur testeValide(String user, String pass, String interim, String service) throws Exception {
        try {
            ...
            return uI.testeValide(user, pass, interim, service);
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new Exception(ex.getMessage());
        }
