...
public class CGenUtil {
    ...
    public static void setValChamp(ClassMAPTable e, Field nomChamp, Object valeur) throws Exception {
        ...
        try {
            if (valeur != null) {
                Method method = e.getClass().getMethod(nomMethode, paramT);
                Object result = method.invoke(e, args);
                // Use or check the result if needed
            }
        ...
        while (dr.next()) {
            if (ii >= iInf && ii <= iSup) {
                ...
                for (int i = 0; i < listeChamp.length; i++) {
                    ...
                    String type = listeChamp[i].getTypeColonne();
                    if ((type.compareToIgnoreCase("NUMBER") == 0) && (listeChamp[i].getPrecision() == 0)) {
                        setValChamp(ex, listeChamp[i].getNomClasse(), new Integer(dr.getInt(listeChamp[i].getNomColonne())));
                    }
                    if (((type.compareToIgnoreCase("NUMBER") == 0) && (listeChamp[i].getPrecision() > 0)) || (type.compareToIgnoreCase("FLOAT") == 0)) {
    ...
    public static HashMap<String,Vector> transformeDataReader2D(ResultSet dr, ClassMAPTable e,String attr2D, Connection c) throws Exception {
        ...
        while (dr.next()) 
        {
            ...
            for (int i = 0; i < listeChamp.length; i++) {
                ...
                String type = listeChamp[i].getTypeColonne();
                if ((type.compareToIgnoreCase("NUMBER") == 0) && (listeChamp[i].getPrecision() == 0) && (listeChamp[i].getTypeJava().compareToIgnoreCase("java.lang.String") != 0)) {

                    setValChamp(ex, listeChamp[i].getNomClasse(), new Integer(dr.getInt(listeChamp[i].getNomColonne())));
                }
                if ((((type.compareToIgnoreCase("NUMBER") == 0) && (listeChamp[i].getPrecision() > 0)) || (type.compareToIgnoreCase("FLOAT") == 0)) && (listeChamp[i].getTypeJava().compareToIgnoreCase("java.lang.String") != 0)) {
                    setValChamp(ex, listeChamp[i].getNomClasse(), new Double(dr.getDouble(listeChamp[i].getNomColonne())));
                }
                if ((type.compareToIgnoreCase("VARCHAR2") == 0) || (type.compareToIgnoreCase("CHAR") == 0)) {
                    setValChamp(ex, listeChamp[i].getNomClasse(), dr.getString(listeChamp[i].getNomColonne()));
...
//        String nomTable = e.getNomTableSelect();
        ...
        while (dr.next()) {
            ...
            for (int i = 0; i < listeChamp.length; i++) {
                ...
                if ((type.compareToIgnoreCase("NVARCHAR2") == 0) || (type.compareToIgnoreCase("CHAR") == 0)) {
                    setValChamp(ex, listeChamp[i].getNomClasse(), dr.getString(listeChamp[i].getNomColonne()));
                }
                if (type.compareToIgnoreCase("DATE") == 0) {
                    setValChamp(ex, listeChamp[i].getNomClasse(), dr.getDate(listeChamp[i].getNomColonne()));
                }
                if (type.toLowerCase().contains("Timestamp".toLowerCase())) {
                    setValChamp(ex, listeChamp[i].getNomClasse(), dr.getTimestamp(listeChamp[i].getNomColonne()));
                }
            }
            ex.setNomTable(e.getNomTableSelect());
